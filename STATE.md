...
- **Verification DONE** (owner-confirmed **2 partners** with official logos + links): CIM Global Financial → `cimglobalfinancial.com/verification?tab=document` · CoverTech Insurance → `covertechinsurance.com/capimax-ecosystem`. Official brand PNGs downloaded → `assets/partner-logos/` (downscaled to 256², rendered as contained squares via `PartnerLogo`→`<Image>`). Cards show the live "Verify externally" button. Assurax dropped; `code` left null (optional). (Old SVG `brandLogos.js` now unused/unimported.)
- **STILL BLOCKED ON OWNER DATA:** WhatsApp number + Live Chat provider · About Us copy review.
