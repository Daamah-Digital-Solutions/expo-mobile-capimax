...
- **Verification links DONE** (owner-confirmed **2 partners**): CIM Global Financial → `cimglobalfinancial.com/verification?tab=document` · CoverTech Insurance → `covertechinsurance.com/capimax-ecosystem` (reuses the `hcc` logo; Assurax dropped). Cards show the live "Verify externally" button. (`code` left null — optional.)
- **STILL BLOCKED ON OWNER DATA:** WhatsApp number + Live Chat provider · About Us copy review · (confirm CoverTech's own logo if the `hcc` file isn't it).
