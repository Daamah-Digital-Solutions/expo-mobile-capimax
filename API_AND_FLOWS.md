...
8. ✅ **RESOLVED (Phase 4 final).** ALL FIVE methods run **LIVE** (no sandbox/test-mode gating) and ALL FIVE run the **contract-create + typed e-signature** step, matching the web's shared `handlePaymentSuccess`. Bank/crypto-manual/NovaPay keep their pending payment **status** (`Processing`/`Pending Verification`) but the contract step still happens; their resolved `transaction_id` (real id or the web's sentinels `BANK_TRANSFER`/`NOVAPAY`/`Pending Verification`) is passed to `contracts/create`. (Details in mobile `STATE.md §7b`.)
